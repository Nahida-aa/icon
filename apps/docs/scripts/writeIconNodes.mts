...
import { readSvgDirectory } from "@xaa/build-icons/helpers/readSvgDirectory";
import { renderIconsObject } from '@xaa/build-icons';
import { toCamelCase } from '@xaa/build-icons/utils/toCamelCase';
const currentDir = process.cwd();
const ICONS_DIR = path.resolve(currentDir, '../../icons');
const svgFiles = await readSvgDirectory(ICONS_DIR);
const {icons} = await renderIconsObject(svgFiles, ICONS_DIR, true);

const iconNodesDirectory = path.resolve(currentDir, 'src/data', 'iconNodes');
